...
public :
  ...
  static void RunSrtTest () {
    ...
    mocha::Profiler insert_profiler( stderr , "HashMap::Insert" );
    ...
    insert_profiler.End();
    mocha::Profiler find_profiler( stderr , "HashMap::Find" );
    find_profiler.Begin();
    int all = all_count - 1;
    while ( all > 0 ) {
      count++;
...
private:
  ...
  static mocha::HashMap<const char*,const char*> hash_;
...
mocha::HashMap<const char*,const char*> HashAutoTester::hash_;
...
int main ( int argc , char** argv ) {
  if ( argc < 3 ) {
    ...
    mocha::HashMap<std::string>::EntryIterator it = hash.Entries();
    while ( it.HasNext() ) {
      printf( "%s\n" , it.Next()->Value().c_str() );
    }
    for ( int i = 0; i < 100; i++ ) {
      char tmp[100];
      sprintf( tmp , "%dabcdefg%d" , i , i );
      mocha::HashMap<std::string>::HashEntry entry = hash.Find( tmp );
      if ( !entry->IsEmpty() ) {
        printf( "%s\n" , entry->Value().c_str() );
      }
    }*/
