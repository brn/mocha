  ...
  while ( begin != end ) {
    ...
    while ( expression_begin != expression_end ) {
      ...
      if ( val->value_type() == Literal::kPrivateProperty ) {
        if ( exp == 0 ) {
          exp =  AstUtils::CreateArrayAccessor( name->Clone() , val->node()->Clone() , literal_->line_number() );
        } else {
          exp =  AstUtils::CreateArrayAccessor( exp , val->node()->Clone() , literal_->line_number() );
        }
