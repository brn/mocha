 ...
 public :
  ...
  void Run() {
    ...
    for (; it != fs_watcher_->map_.end(); ++it) {
      struct kevent kev;
      EV_SET(&kev, it->second->handle()->fd(), EVFILT_VNODE, EV_ADD | EV_CLEAR, vnode_events, 0, it->second.Get());
    ...
    while (!is_exit_) {
      ...
      if (ev_count > 0) {
        for( int i = 0; i < ev_count; i++) {
          ...
          if (e->IsExist()) {
            if (e->IsModified()) {
              fs_watcher_->NotifyForKey(FSWatcher::kModifiy, e);
            }
            if (e->IsUpdate()) {
              fs_watcher_->NotifyForKey(FSWatcher::kUpdate, e);
            }
          } else {
            if (e->IsModified()) {
              fs_watcher_->NotifyForKey(FSWatcher::kDelete, e);
            }
          }
        }
      }
...
void FSWatcher::AddWatch(const char* path) {
  FSEventHandle handle = FSEventHandle(new FSEvent(path, this));
  map_.insert(FSEventPair(path, handle));
}
