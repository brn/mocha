!function() {
  ...
  !function () {
    ...
    !function () {
      function concat() {
        ...
        for (var i = 0,length = arguments.length;i<length;i ++ ){
          ...
          if (Object.isArray(item) && !('callee' in item))for (var j = 0,arrayLength = item.length;j<arrayLength;j ++ )
          array.push(item[j]);
    ...
    var Hash = Class.create(Enumerable,function () {
          ...
          function get(key) {
            if (this._object[key] !== ({})[key])return this._object[key];
          }
    ...
    } else Prototype.Browser.Gecko && /rv:1\.8\.0/.test(navigator.userAgent)?Element.Methods.setOpacity = function (element,value) {
      element = $(element);
    ...
    Element.extend = function () {
      ...
      if (Prototype.BrowserFeatures.SpecificElementExtensions){
        ...
        if (HTMLOBJECTELEMENT_PROTOTYPE_BUGGY)return function (element) {
    ...
    Element.addMethods = function (methods) {
      ...
      if (F.SpecificElementExtensions)for (var tag in Element.Methods.ByTag){
    ...
    !function () {
      ...
      } else Prototype.Browser.Webkit && (cumulativeOffset = function (element) {
        element = $(element);
    ...
    Prototype.Selector = function () {
      ...
      function find(elements,expression,index) {
        ...
        for (i = 0;i<length;i ++ )if (match(elements[i],expression) && index == matchIndex ++ )return Element.extend(elements[i]);
      }
      function match() {
    ...
    !function () {
          ...
          if (elem){
            ...
            while (elem){
              ...
              if (elem.nodeType === 1){
                ...
                if (typeof cur !== "string")if (elem === cur){
      ...
      var Sizzle = function (selector,context,results,seed) {
            ...
            if (toString.call(checkSet) === "[object Array]")if (!prune)results.push.apply(results,checkSet);
             else if (context && context.nodeType === 1)for (var i = 0;checkSet[i] != null;i ++ )
        ...
        while (expr && set.length){
          ...
          for (var type in Expr.filter)if ((match = Expr.match[type].exec(expr)) != null){
            ...
            if (item){
              ...
              if (inplace && found != null)pass?anyFound = true : curLoop[i] = false;
               else if (pass){
      ...
      try {
        ...
        makeArray = function (array,results) {
          ...
          if (toString.call(array) === "[object Array]")[].push.apply(ret,array);
           else {
            if (typeof array.length === "number")for (var i = 0,l = array.length;i<l;i ++ )
            ret.push(array[i]);
             else for (var i = 0;array[i];i ++ )
            ret.push(array[i]);
          }
          return ret;
        };
      ...
      !function () {
        ...
        div.getElementsByTagName("*").length>0 && (Expr.find.TAG = function (match,context) {
          ...
          if (match[1] === "*"){
            ...
            for (var i = 0;results[i];i ++ )results[i].nodeType === 1 && tmp.push(results[i]);
    ...
    Form.Element.Serializers = function () {
      ...
      function select(element,value) {
        ...
        for (var i = 0,length = element.length;i<length;i ++ ){
          ...
          if (single)if (currentValue == value){
    ...
    !function () {
      ...
      function stopObserving(element,eventName,handler) {
        ...
        while (i -- )if (responders[i].handler === handler){
        ...
        if (eventName.include(':'))if (element.removeEventListener)element.removeEventListener("dataavailable",responder,false);
         else {
      ...
      function observe(element,eventName,handler) {
        ...
        if (eventName.include(':'))if (element.addEventListener)element.addEventListener("dataavailable",responder,false);
         else {
