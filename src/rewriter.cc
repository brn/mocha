...
#define VISITOR_IMPL(name) Rewriter::operator()( name* ast_node )
#define ACCEPT( ast )                           \
  if ( ast != 0 )                               \
    ast->Accept(this)
...
VISITOR_IMPL( AstRoot ) {
  std::list<AstTypeBase*>::iterator ITERATOR(ast_node->Tree());
  while ( begin != end ) {
    (*begin)->Accept ( this );
    ++begin;
  }
...
VISITOR_IMPL(AstTree) {
  std::list<SourceBlock*>::iterator ITERATOR(ast_node->List());
  while ( it != end ) {
    ACCEPT((*it));
    it++;
  }
