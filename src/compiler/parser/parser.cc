...
Parser::Parser( ParserConnector* connector , ErrorReporter* reporter , const char* filename )
    : filename_( filename ) , connector_( connector ) , reporter_( reporter ) {}
Parser::~Parser() {}
    ...
    case Token::JS_CONST : {
      ...
      if ( type == Token::JS_CLASS ) {
        ...
        if ( type == '(' || type == Token::JS_FUNCTION_GLYPH ||
          ...
          result = ParseVariableDecl_( false );
        }
      }
      break;
    }
...
AstNode* Parser::ParseImportStatement_() {
  ...
  if ( r_from->GetType() == Token::JS_IDENTIFIER &&
    ...
    if ( !semicolon->HasLineBreakBefore() && type != ';' ) {
...
AstNode* Parser::ParseVariableDecl_( bool is_noin ) {
  ...
  while ( 1 ) {
     ...
     * if not, it's error.
     */
    if ( next_type != ',' && ( next->HasLineBreakBefore() || next_type == ';' ) ) {
      break;
    } else if ( next_type != ',' ) {
      SYNTAX_ERROR( "parse error got unexpected token "
                    << TokenConverter( next )
...
AstNode* Parser::ParseObjectPattern_() {
  ...
  while ( brace_count > 0 ) {
    if ( maybe_colon == '}' ) {
      ...
      if ( !elem->IsEmpty() ) {
        elem->FirstChild()->AddChild( elem->FirstChild()->Clone() );
      }
      brace_count--;
    } else if ( maybe_colon == '{' ) {
      brace_count++;
...
AstNode* Parser::ParseYieldExpression_( bool is_noin ) {
  ...
  if ( type == Token::JS_YIELD ) {
    token = Advance_();
    type = token->GetType();
    if ( token->HasLineBreakAfter() || Seek_()->GetType() == ';' ) {
...
AstNode* Parser::ParseCallExpression_() {
  ...
  if ( type == '(' ) {
    ...
    if ( member->NodeType() == AstNode::kCallExp ) {
      exp = member->CastToExpression()->CastToCallExp();
      exp->Call();
    } else {
      exp = ManagedHandle::Retain( new CallExp( CallExp::kNormalAccessor ) );
      exp->AddChild( member );
      exp->Call();
    }
    token = Seek_();
    ...
    if ( type == '.' || type == '[' ) {
      while ( 1 ) {
        if ( type == '[' ) {
          Property* accessor = ManagedHandle::Retain( new Property( Property::kBracket ) );
          AstNode* node = ParseExpression_( false );
          CHECK_ERROR( member );
          ...
          exp->AddChild( accessor );
        } else if ( type == '.' ) {
          int next = Seek_()->GetType();
          Property* accessor;
          if ( next == '{' ) {
            accessor = ManagedHandle::Retain( new Property( Property::kExtend ) );
            Advance_();
            AstNode* node = ParseObjectLiteral_();
            CHECK_ERROR( member );
            accessor->AddChild( node );
          } else {
            accessor = ManagedHandle::Retain( new Property( Property::kDot ) );
            Advance_();
            AstNode* node = ParseLiteral_();
            CHECK_ERROR( member );
            ...
            if ( !maybeIdent || maybeIdent->ValueType() != ValueNode::kIdentifier ) {
              SYNTAX_ERROR( "parse error got unexpected token "
                            << TokenConverter( token )
                            << " in 'call expression' expect 'identifier'\nin file "
                            << filename_ << " at line " << token->GetLineNumber() );
              END(CallExpressionError);
              return exp;
            }
...
AstNode* Parser::ParseFunctionDecl_( bool is_const ) {
  ...
  if ( type == Token::JS_FUNCTION_GLYPH || type == Token::JS_FUNCTION_GLYPH_WITH_CONTEXT ) {
    ...
    if ( type == '{' ) {
      Advance_();
      AstNode* node = ParseSourceElements_();
      CHECK_ERROR( node );
      fn->Append( node );
      token = Seek_();
      type = token->GetType();
...
AstNode* Parser::ParseArrowFunctionBody_( Function* fn ) {
  ...
  if ( token_type == '{' ) {
    Advance_();
    while ( token_type != '}' || IsEnd( token_type ) ) {
    ...
    if ( token_type != '}' ) {
      SYNTAX_ERROR( "parse error got unexpected token "
                    << TokenConverter( token )
                    << " in 'arrow function expression' expect '}'\nin file "
                    << filename_ << " at line " << token->GetLineNumber() );
    }
    Advance_();
  } else {
