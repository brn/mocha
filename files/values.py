...
'first_step_guide.html' : {
        ...
        'en' : {
            ...
            'comp_setting_versions_sub' : 'Define version symbols. This value is referenced by the @version operator.',
            'comp_setting_libs_sub' : 'Set libraries which are combined at the compilation time. The libraries are combined by array order.',
            ...
            'comp_setting_module_dir_sub' : 'Set module directories. This directories are used by \'import\', \'@include\' and \'@source\' to find files.',
            ...
            'comp_setting_sub32' : 'Specify the directories that used at searching modules.',
            'comp_setting_sub33' : 'Specify the versions.',
            'comp_setting_sub34' : 'Specify whether create the per file\'s scope or not. This value is true by default.',
            'comp_setting_sub35' : 'Specify whether create the global scope or not. This value is false. by default',
            'comp_setting_libs_exp' : 'Specify libraries.',
            ...
            'command_sub12' : 'Compile the specified file. A predicate must be a function or string or regular expression. A option can specify the new compilation option, if necessary. If the new option is passed, that will merged with old one. And also a option can specify string \'debug\' or \'release\'. If option string is \'debug\', mocha compile files with options which set debug, prettyPrint. If option string is \'release\', mocha compile files with options which set no debug, compress.',
            ...
            'comp_dir_source_exp': 'Load a file and extract at this point without compilation.',
            'comp_dir_sub7' : 'Load and compile a file. But modules are not exported. This directives purpose is to import javascript files which are written by ecma262.',
            'comp_dir_sub8' : 'Extract like this.',
            'comp_dir_sub9' : 'In the debug mode, x = 1',
            'comp_dir_sub10' : 'In the release mode, x = 2',
            'comp_dir_source_sub' : 'The code of jQuery is inserted In the position of @include without compilation.',
            'comp_dir_sub11' : 'The jQuery is loaded and compiled, but no modules are exported.',
