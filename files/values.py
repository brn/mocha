...
詳しくはfor ofを参照してください。""",
        ...
        'en' : {
            ...
            'class_detail_sub' : 'In mocha, the class behave as syntax sugar of the definition of the constructor function and prototypes, so it has 100% compatibility to existing javascript\'s class which declared as a function and prototypes. And the overhead of a class is supressed as much as possible, but to realize the private field, the class syntax has a little overhead.',
            ...
            'trait_detail_sub1' : """The trait purpose is that inject behavior by implementation inheritance. And also the trait behave like a abstract class in terms of enforce implementation of methods, to enforce implementation of methods, use a \'require\' attribute, like this, require foo.
And the trait can compose traits or classes through declaration of \'mixin\' attributes, and can rewrite member names or disable member to set \'with\' or \'without\' attributes in a \'mixin\' attribute, like mixin ... with &lt;method_name&gt; as &lt:new_name&gt;
The modifier like public, private behave same as class.""",
            'trait_interface_title' : 'Interface',
            'trait_interface_sub1' : 'Declare a trait.',
            'trait_interface_sub2' : 'Mixin traits. If same name members are there, it overwritten by after definition. And If with ~ as is set, a member name is rewrite as new member name, and If without ~ is set, the specific member is not mixined.',
